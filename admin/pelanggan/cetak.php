...
$pdf->Cell(19, 1, 'Laporan Data Pelanggan', 0, 1, 'C');
$pdf->Ln();

$pdf->SetFont('Arial', 'B', 7);
$pdf->SetFillColor(240, 240, 240);
$pdf->Cell(1, 1, 'No', 1, 0, 'C', true);
$pdf->Cell(2, 1, 'Id Pelanggan', 1, 0, 'C', true);
$pdf->Cell(4, 1, 'Nama Pelanggan', 1, 0, 'C', true);
$pdf->Cell(4, 1, 'Username', 1, 0, 'C', true);
$pdf->Cell(4, 1, 'Email', 1, 0, 'C', true);
$pdf->Cell(4, 1, 'No Telepon', 1, 1, 'C', true);

//menjalankan kueri mysql
$select = mysqli_query($connection, "SELECT * FROM t_pelanggan");
...
if (mysqli_num_rows($select) > 0) {
    ...
    while ($hasil = mysqli_fetch_array($select)) {
        ...
        $pdf->Cell(2, 1, $hasil['id_pelanggan'], 1, 0, 'C');
        $pdf->Cell(4, 1, $hasil['nama_pelanggan'], 1, 0, 'L');
        $pdf->Cell(4, 1, $hasil['username'], 1, 0, 'L');
        $pdf->Cell(4, 1, $hasil['email'], 1, 0, 'L');
        $pdf->Cell(4, 1, $hasil['no_telp'], 1, 1, 'C');
    }
} else {
    $pdf->SetFont('Arial', '', 7);
