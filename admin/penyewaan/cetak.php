...
$pdf->Cell(2, 1, 'Id Penyewaan', 1, 0, 'C', true);
$pdf->Cell(2, 1, 'Nama Pelanggan', 1, 0, 'C', true);
$pdf->Cell(2, 1, 'No Telepon', 1, 0, 'C', true);
$pdf->Cell(2, 1, 'Email', 1, 0, 'C', true);
$pdf->Cell(4, 1, 'Paket Alat Musik', 1, 0, 'C', true);
$pdf->Cell(1, 1, 'Tanggal Mulai Sewa', 1, 0, 'C', true);
$pdf->Cell(1, 1, 'Tanggal Pengembalian', 1, 0, 'C', true);
$pdf->Cell(2, 1, 'Jumlah Hari', 1, 0, 'C', true);
$pdf->Cell(2, 1, 'Biaya', 1, 1, 'C', true);

//menjalankan kueri mysql
$select = mysqli_query($connection, "SELECT * FROM t_penyewaan");
...
if (mysqli_num_rows($select) > 0) {
    ...
    while ($hasil = mysqli_fetch_array($select)) {
        ...
        $pdf->Cell(2, 1, $hasil['id_penyewaan'], 1, 0, 'C');
        $pdf->Cell(2, 1, $hasil['nama_pelanggan'], 1, 0, 'L');
        $pdf->Cell(2, 1, $hasil['no_telp'], 1, 0, 'L');
        $pdf->Cell(2, 1, $hasil['email'], 1, 0, 'L');
        $pdf->Cell(4, 1, $hasil['paket'], 1, 0, 'C');
        $pdf->Cell(1, 1, $hasil['tgl_sewa'], 1, 0, 'C');
        $pdf->Cell(1, 1, $hasil['tgl_pengembalian'], 1, 0, 'C');
        $pdf->Cell(2, 1, $hasil['jumlah_hari'], 1, 0, 'C');
